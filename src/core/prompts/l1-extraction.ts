...
4. 范围标注：每条记忆必须输出 "scope"，取值只能是 "global"、"project" 或 "session"。
   - global：跨项目长期成立的身份、偏好、能力、稳定规则。
   - project：明确限定在当前项目、仓库、工作区、业务域、PR 或 issue 的要求。
   - session：只对本轮、本次、当前任务、临时排查或一次性操作成立的上下文。
   - 场景受限、项目受限、本次任务限定的要求不得泛化为全局，必须标为 project 或 session。
...
3. 指令记忆 (type: "instruction")
   - 定义：用户对 AI 提出的行为规则、格式偏好、语气控制。必须结合 scope 判断长期性。
   - 提取句式："用户要求/希望 AI 以后回答时..."
   - 触发词：以后都、从现在开始、记住、必须。
   - 打分 (priority)：-1（极其严格的全局死命令）；90-100（核心行为规则）；70-80（重要要求）；<70（临时要求，直接丢弃）。
   - 关键约束：只有明确跨项目、长期稳定的规则才可标为 scope="global"；当前项目/当前仓库/这次任务里的规则必须标为 project 或 session。
...
- 所有类型：scope 必须放在顶层字段，不要只放在 metadata 中。
- 其他类型或无法确定时间：输出空对象 {}
