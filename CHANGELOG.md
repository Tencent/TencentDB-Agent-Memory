...
#### OpenClaw 宿主兼容
- **Windows OpenClaw npm archive extraction compatibility** ([#148](https://github.com/TencentCloud/TencentDB-Agent-Memory/issues/148)): rename the packaged license file from `LICENSE` to `LICENSE-MIT` so npm tarballs no longer contain `package/LICENSE`, avoiding the Windows extraction failure while still shipping the MIT license text.
